...
exports.getAllPosts = (data, callback) => {
    db.query(
        `SELECT p.id AS postId, p.description, p.datetimeCreated, p.likeCount, p.dislikeCount, p.addedByUserId,
...
exports.likePost = (data, callback) => {
    db.query(
        ...
        (error, results, fields) => {
            if (error) return callback(error);
            if (results.affectedRows === 1) return callback(null, `Like Successful`);
            else return callback(new Error("Invalid post"));
    });
};
...
exports.deletePost = (data, callback) => {
    db.query(
        ...
        (error, results, fields) => {
            if (error) return callback(error);
            else if (results.affectedRows === 1) return callback(null, "Post Deleted Successfuly.");
            else return callback(new Error("Invalid post"));
    });
};